...
#否则，请检查$SCRATCH_ROOT env变量是否存在并且是否为目录。

#否则使用默认值（<proj_root>/scratch）
...
def resolve_scratch_root(arg_scratch_root, proj_root):
    ...
    if not arg_scratch_root:
        if scratch_root is None:
            ...
            #临时空间可以安装在网络驱动器上的文件系统（如NFS）中。

            #如果网络关闭，可能会导致访问检查挂起。那就跑一趟吧
...
def resolve_branch(branch):
    ...
    如果在命令行上传递了--branch参数，则分支

    参数是要使用的分支名称。否则就没有了，我们使用git
...
def resolve_proj_root(args):
    ...
    根据调用DVSim的方式更新proj_root。
    如果设置了远程开关，则选择划痕区域中的一个位置作为
    新项目根。整个回购协议被复制到这个位置。否则
    proj_root是使用get_proj_root（）方法发现的，除非用户
    在命令行上覆盖它。
    此函数返回更新后的项目根src和目标路径。如果
    --未设置远程开关，目标路径与src相同
    路径同样，如果设置了“干运行”。
    '''
