...
class StringWithUseFlags(str):
    ...
    def parse(self, flags):
        if self.exprs is None:
            #print(str(self))
            temp_str = str(self).format(**os.environ)
            #print(temp_str)
            self.exprs = Exprs(temp_str)
    ...
    def __init__(self, tree):
        for k, v in sorted(tree.items()):
            ...
            if k in self.members:
                ...
                logger.debug("Unknown item " + k + " in section " + self.__class__.__name__)
...
class Core:
    ...
    def __init__(
        self, core_file, cache_root="", generated=False, resolve_env_vars=False, core_root = None
    ):
    ...
    def export(self, dst_dir, flags={}):
        ...
        for f in src_files:
            ...
            if not os.path.isabs(f):
                #abs_path_1 = os.path.join(self.core_root, f)
                #abs_path_1 = abs_path_1.replace('\\','/')
                #abs_path_2 = os.path.join(self.files_root, f).replace('\\','/')
                #print("Check %s in :\n\t%s\n\t%s" % (f, abs_path_1, abs_path_2))
