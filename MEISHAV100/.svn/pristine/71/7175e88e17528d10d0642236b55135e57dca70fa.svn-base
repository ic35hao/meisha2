...
class Edatool(object):
    ...
    def _add_include_dir(self, f, incdirs, force_slash=False):
        if f.get("is_include_file"):
            ...
            _incdir = f.get("core_root") or os.path.dirname(f["name"]) or "."
            if force_slash:
    ...
    def _get_fileset_files(self, force_slash=False):
        class File:
            def __init__(self, name, file_type, logical_name, incdir = None):
                self.name = name
    ...
    def _write_fileset_to_f_file(
        ...
        with open(output_file, "w") as f:
            ...
            for src_file in src_files:
                ...
                elif filter_func(src_file):
                    if(src_file.file_type  == "fileList"):
