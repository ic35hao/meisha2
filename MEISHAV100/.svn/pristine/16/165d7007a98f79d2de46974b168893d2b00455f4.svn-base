...
`define chk_prot_addr_mask_align \
  MaskWidth'(a_mask << (8 - a_addr[SizeWidth-1:0])) == 0
...
class tl_seq_item extends uvm_sequence_item;
  ...
  `uvm_object_utils_begin(tl_seq_item)
    ...
    `uvm_field_int  (remain_byte_cnt,     UVM_DEFAULT)
  ...
  virtual function string convert2string();
    ...
    str = {$sformatf("a_addr = 0x%0h ", a_addr),
           ...
           $sformatf("remain_byte_cnt = 0d%0d ", remain_byte_cnt),
           $sformatf("a_valid_delay = 0x%0h ", a_valid_delay),
           $sformatf("d_valid_delay = 0x%0h ", d_valid_delay),
           $sformatf("a_valid_len = 0x%0h ", a_valid_len),
           $sformatf("d_valid_len = 0x%0h ", d_valid_len),
