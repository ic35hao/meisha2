...
class Edalizer:
    ...
    def run(self):
        ...
        os.environ['ROOT_PATH'] = "/d/work/opentitan/opentitan/hw/ip/axi_xbar/rtl"
        self.create_edam()
    ...
    def create_edam(self):
        ...
        for core in self.cores:
            ...
            for file in core.get_files(_flags):
                ...
                _f["true_name"] = core.core_root + '/' + file["name"] # need be before
                _f["name"] = os.path.join(core.core_root, file["name"])
                #_f["name"] = str(_name)
                _f["core"] = str(core.name)
                _f["core_root"] = core.core_root
                if file.get("include_path"):
                    #_f["include_path"] = os.path.join(rel_root, file["include_path"])
                    _f["include_path"] = os.path.join(core.core_root, file["include_path"])
                _files.append(_f)
