...
class dv_base_test #(type CFG_T = dv_base_env_cfg,
  ...
  virtual function void build_phase(uvm_phase phase);
    ...
    if(cfg == null) begin
      ...
      `uvm_warning(`gfn, {"use default cfg type ", cfg.get_type_name()})
    end
    //`uvm_warning(`gfn, {"use default cfg type ", cfg.get_type_name()})
    cfg.initialize();
    //`uvm_warning(`gfn, {"use default cfg type ", cfg.get_type_name()})
  ...
  virtual function void end_of_elaboration_phase(uvm_phase phase);
    ...
    //set_report_max_quit_count(max_quit_count);
...
virtual task trigger_env_clk_rst();
    `uvm_warning(`gfn, {"need overrite trigger_env_clk_rst task in testcase top"})
endtask

virtual task wait_clk_rst();
    `uvm_warning(`gfn, {"need overrite wait_clk_rst task in testcase top"})
endtask
