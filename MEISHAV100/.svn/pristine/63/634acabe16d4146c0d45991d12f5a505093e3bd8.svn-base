...
class CoreManager:
    def __init__(self, config, build_root, resolve_env_vars=False):
        self.config = config
        self.db = CoreDB()
    ...
    def extract_core(self, temp_file, temp_core_file):
        # 从通用文件头解析yaml格式，并导出到临时路径下
        core_line = []
        is_core_file = False
        begin_get_core = False
        with open(temp_file) as f:
            logger.info(
                                "Core file {} extract".format(temp_file)
                            )
            lines = f.readlines()
            for l in lines:
                #l = l.strip()
                ...
                l = l.lstrip("/*")
                l = l.rstrip("*/")
                if(l.lower().find("yaml_header_begin") >= 0):
                    begin_get_core = True
                elif(begin_get_core == True):
                    if(l.lower().find("yaml_header_end") >= 0):
                        is_core_file = True
                        break
                    else:
                        core_line.append(l)
                elif(l.lower().find("yaml_header_end") >= 0):
                    is_core_file = False
                    logger.error(
                                "Core file {} have yaml_header_end, but not have yaml_header_begin"
                                ", which is not supported ".format(temp_file)
                            )
                    break
                else:
                    pass
        if(is_core_file is True):
            with open(temp_core_file, "w+") as f:
                f.write("".join(core_line))
                logger.warning(
                                "Write Core file {} Success".format(temp_core_file)
                            )
                pass
        elif(begin_get_core is True):
            logger.error(
                                "Core file {} have yaml_header_begin, but not have yaml_header_end"
                                ", which is not supported ".format(temp_file)
                            )
        else:
            #logger.warning("Core file {} is core file, but not have yaml_header_end, which is not supported ".format(temp_file))
            pass
    ...
    def find_cores(self, library, ignored_dirs):
        ...
        for root, dirs, files in os.walk(path, followlinks=True):
            ...
            for f in files:
                # 拆分文件名，查找匹配*.core.*类似格式文件，均认为是core文件
                fe_list = f.split(".")
                if(len(fe_list) < 2):
                    pass
                elif((len(fe_list) > 2) and (fe_list[-2] == "core")):
                    core_file = self.build_root + "/" + ".".join(fe_list[0:-1])
                    self.extract_core(os.path.join(root, f), core_file)
                    try:
                        ...
                        core = Core(
                            ...
                            core_root = os.path.dirname(os.path.join(root, f))
                        )
                        ...
                        logger.warning(e)
                elif(fe_list[-1] == "core"): #if f.endswith(".core"):
