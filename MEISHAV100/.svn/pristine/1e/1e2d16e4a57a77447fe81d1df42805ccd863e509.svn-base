...
class dv_base_scoreboard #(type RAL_T = dv_base_reg_block,
  ...
  uvm_blocking_get_port #(uvm_sequence_item) exp_port[string];
  uvm_blocking_get_port #(uvm_sequence_item) act_port[string];
  bit obj_raised      = 1'b0;
  ...
  virtual function void pre_abort();
    ...
    if (has_uvm_fatal_occurred() && !under_pre_abort && m_current_phase.is(uvm_run_phase::get())) begin
